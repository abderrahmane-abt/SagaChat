    ...
    AlertDialog(
        ...
        text = {
            Column(
                ...
                Text(
                    "Your device shows signs that lower the protection floor for SagaChat. The app will keep running, but read this once.",
                ...
                if (rootEvidence.isNotEmpty()) {
                    ...
                    Text(
                        "Root access: another app on this device with root can read SagaChat's encrypted files and, in principle, extract session data while the app is running. Your PIN is still hardware-sealed by Android Keystore.",
                        style = MaterialTheme.typography.bodyMedium,
                    )
                ...
                if (tamperEvidence.isNotEmpty()) {
                    Spacer(Modifier.height(12.dp))
                    Text(
                        "Hooking framework detected (Xposed / LSPosed / similar). Modules can intercept method calls inside SagaChat and observe data in memory regardless of encryption.",
                        style = MaterialTheme.typography.bodyMedium,
                    )
                    Spacer(Modifier.height(6.dp))
                    Text(
