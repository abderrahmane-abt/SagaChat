...
enum class LogLevel(val color: Color, val prefix: String) {
    DEBUG(Color(0xFF2196F3), "DEBUG"), INFO(Color(0xFF4CAF50), "INFO"), WARNING(
        Color(0xFFFFC107),
        "WARN"
    ),
    ERROR(Color(0xFFF44336), "ERROR"), CRITICAL(Color(0xFFD32F2F), "CRIT")
}
...
object VaultLogger {
    ...
    fun search(query: String): List<LogEntry> {
        return _logs.filter {
            it.message.contains(query, ignoreCase = true) || it.tag.contains(
                query,
                ignoreCase = true
            )
        }
