...
private fun processWideAccents(input: String): String {
    var r = input
    r = Regex("""\\widehat\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c -> "$c\u0302" }.joinToString("")
    }
    r = Regex("""\\widetilde\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c -> "$c\u0303" }.joinToString("")
    }
    return r
}
...
private fun processOverUnderline(input: String): String {
    var r = input
    r = Regex("""\\overline\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c -> "$c\u0305" }.joinToString("")
    }
    r = Regex("""\\underline\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c -> "$c\u0332" }.joinToString("")
    }
    return r
}
...
private fun processCancel(input: String): String {
    var r = input
    r = Regex("""\\cancel\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c -> "$c\u0336" }.joinToString("")
    }
    r = Regex("""\\[bx]cancel\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c -> "$c\u0336" }.joinToString("")
    }
    return r
}
...
private fun processSuperscripts(input: String): String {
    var r = input
    r = Regex("""\^\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c ->
            superscriptMap[c] ?: c
        }.joinToString("")
    }
    r = Regex("""\^([a-zA-Z0-9])""").replace(r) {
        superscriptMap[it.groupValues[1][0]]?.toString() ?: "^${it.groupValues[1]}"
    }
    return r
}
...
private fun processSubscripts(input: String): String {
    var r = input
    r = Regex("""_\{([^}]+)\}""").replace(r) {
        it.groupValues[1].map { c -> subscriptMap[c] ?: c }.joinToString("")
    }
    r = Regex("""_([a-zA-Z0-9])""").replace(r) {
        subscriptMap[it.groupValues[1][0]]?.toString() ?: "_${it.groupValues[1]}"
    }
    return r
}
...
private fun processStacking(input: String): String {
    ...
    r =
        Regex("""\\overset\{([^}]+)\}\{([^}]+)\}""").replace(r) { "${it.groupValues[2]}^{${it.groupValues[1]}}" }
    r =
        Regex("""\\underset\{([^}]+)\}\{([^}]+)\}""").replace(r) { "${it.groupValues[2]}_{${it.groupValues[1]}}" }
    return r
}
    ...
    for (mt in listOf("matrix", "pmatrix", "bmatrix", "vmatrix", "Vmatrix")) {
        r = Regex("""\\begin\{$mt\}(.*?)\\end\{$mt\}""", RegexOption.DOT_MATCHES_ALL).replace(r) {
            ...
            val body = rows.joinToString("\n") { row ->
                "  " + row.split("&").joinToString("  ") { c -> c.trim() }
            }
            if (lb.isNotEmpty()) "$lb\n$body\n$rb" else body
        }
    }
    r = Regex("""\\begin\{align\*?\}(.*?)\\end\{align\*?\}""", RegexOption.DOT_MATCHES_ALL).replace(
        r
    ) {
        it.groupValues[1].trim().split("\\\\\\\\").map { l -> l.trim().replace("&", "") }
