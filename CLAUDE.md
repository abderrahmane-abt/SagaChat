...
| `rag_keyword_v1/` | `tn.rag_keyword.user_key.v2` | BM25 inverted-index records. Records key by `sourceId` (chat-agnostic) — the on-disk `chat_id` field is vestigial and always `""` in new records. Filter to current chat happens in Kotlin via `documentRepo`. |
| `rag_cache_v1/` | `tn.rag_cache.user_key.v2` | Per-source dense embedding blobs from `RAGEngine.exportDoc(docId)`. Records: `(docId, sourceId, fingerprint, blob)`. Replaces the previous plaintext `rag_vector_snapshot_v1.bin` whole-engine snapshot. |
...
Every attached document is stored content-addressed by SHA-256 of its bytes. The RAG stack now has two distinct ID spaces:

- **`sourceId` = sha256 of bytes** — the unit of de-duplication. Used as the engine's docId, the BM25 records' docId, and the cache key.
- **`docMetaId = "<chatId>:<sourceId>"`** — only used in `chat_documents_meta_v1` (per-chat-attachment record). Many docMetaIds can point at the same sourceId.

Same content attached to N chats → exactly **one** dense embedding entry, **one** BM25 entry, **one** source bytes file. Re-attach is metadata-only, no embedding work.

| Path | Sealed under | Contents |
|---|---|---|
| `chat_documents/sources_v2/<sourceId>.bin` | per-file AEAD bound to `(DEK, signerHash, sourceId)` (AAD = sourceId) | Raw doc bytes, content-addressed by sha256. Shared across chats. |
| `chat_documents_meta_v1/` | `tn.chat_documents.user_key.v2` | Per-attachment metadata `(id=docMetaId, chatId, sourceId, name, mime, chunkCount, sizeBytes, addedAt, isDeepIndexed, isRaptorIndexed)`. |
| `rag_keyword_v1/` | `tn.rag_keyword.user_key.v2` | BM25 inverted index. Records keyed by `sourceId` only; the on-disk `chat_id` field is vestigial and always `""`. Native query passes `chat_id=""` (no native filter); Kotlin post-filters hits by the active chat's sourceId set from `documentRepo`. |
| `rag_cache_v1/` | `tn.rag_cache.user_key.v2` | **New encrypted RAG cache.** One record per docId (sourceId or `${sourceId}::ctxN` etc.). Fields: `(docId, sourceId, fingerprint, blob)` where `blob` is `RAGEngine.exportDoc(docId)` — chunk text + binary-quantized vectors + float embeddings + per-doc fingerprint header. Replaces the previous plaintext `rag_vector_snapshot_v1.bin`. |

**Engine is per-active-chat in RAM.** `RagManager.hydrateChat(chatId)` diffs `loadedSources` against the chat's sourceIds, calls `engine.removeDocument` for ones leaving and `engine.importDoc(blob)` for ones arriving (cache lookup keyed by sourceId). Cache miss / fingerprint mismatch falls back to `engine.ingestBytes(...)` + `cache.write(exportDoc(...))`. The dense index in RAM only ever contains the active chat's sources, which means `engine.queryFiltered(q, "")` is naturally chat-scoped — no docId-prefix filtering needed.

`RagManager.attachExisting(currentChatId, source)` is now metadata-only when the cache hits — `documentRepo.addDocument(...)` plus an `ensureSourceInEngine` call that imports the cached blob if the source isn't already loaded. Re-attach across two chats with the same source costs zero embedding seconds.

`RagManager.ingestDocument(...)`: read URI bytes → `sourceId = sha256(bytes)` → write to `SourceFileVault` if new → if cache has fingerprint-matching blobs for `sourceId`, `engine.importDoc` them; else `engine.ingestBytes(bytes, mime, name, sourceId)` → `engine.exportDoc(sourceId)` → `cache.write(...)` → BM25 index for text-like (gated on `keywordIndex.docCount(sourceId) > 0`) → `documentRepo.addDocument(ChatDocument(id="$chatId:$sourceId", …))`.

`RagManager.removeDocument(docMetaId)` decrements the per-chat metadata record. If `documentRepo.countWithSource(sourceId) == 0` after, it tears down everything tied to that sourceId: `engine.removeDocument` (including any `::ctxN` / `::raptor*` subdocs), `cache.removeBySource(sourceId)`, `keywordIndex.removeDocument(sourceId)` (and subdocs), `sourceVault.delete(sourceId)`. Other chats that still share the source keep their cache + engine state untouched.

**Active fingerprint = `model.id` (or `model.path` if id blank).** Stored in each cache record. On `ensureReady` after model swap, `cache.invalidateForeign(activeFingerprint)` drops blobs that were embedded with a different model. Native `engine.importDoc` returns -4 on fingerprint mismatch as a defense-in-depth check; we treat that as "stale, evict + re-embed".

**Migration on first launch with new code:**
- `RagCacheRepository.init` deletes the legacy plaintext `rag_vector_snapshot_v1.bin` (it had no encryption-at-rest — real regression vs the rest of the project).
- `RagKeywordIndex.init` walks BM25 records once; if any record has `chat_id != ""` or `doc_id` containing `:` (legacy `chatId:sourceId` form), it `ragClear`s the collection and lazily re-ingests on the next chat hydrate. Vault dir name `rag_keyword_v1/` is intentionally unchanged — the schema migrates in-place since nothing has shipped to production yet.
- Existing `chat_documents_meta_v1` records keep working as-is (their `sourceId` field was already separate from `id`).
...
2. **Per-query retrieval** — for each query (original + variants): `engine.queryFiltered(q, "")` for dense (engine in-RAM only contains the active chat's sources, so no prefix needed) AND `RagKeywordIndex.query(q, KEYWORD_CANDIDATES = 20)` for BM25 (no chat filter at the C++ layer; Kotlin then filters hits by `documentRepo.getDocumentsForChat(chatId).sourceIds`). Both rankings cap at `DENSE_CANDIDATES / KEYWORD_CANDIDATES = 20`.
...
`RagKeywordIndex` is native — backed by `hxs::RagKeywordIndex` in `hxs/src/main/cpp/rag_keyword.{h,cpp}`. Per-chunk records use the on-disk TAG layout `(1=docId, 2=chatId, 3=sourceId, 4=chunkIndex, 5=text)`. **Kotlin always writes `chat_id=""` and queries with `chat_id=""`**; the field is preserved for binary compatibility but plays no role in scoping. C++ maintains an in-memory `unordered_map<term, vector<Posting{record_id, term_freq}>>` rebuilt at construction by scanning the encrypted records. Tokenizer is ASCII alphanumeric + underscore + UTF-8 bytes ≥0x80 passthrough, lowercased, length 2-64. BM25 params k1=1.2, b=0.75. JNI surface: `nativeRagIngest`, `nativeRagQuery`, `nativeRagRemoveDocument`, `nativeRagClear`, `nativeRagDocCount`.

**BM25 limitation (text-format gate, not a binary blocker):** only mime-text-like docs are BM25-indexed today via `RagManager.isTextLike(...)` — `text/*`, `application/{json,xml,rtf,javascript,yaml}`, or extensions `txt|md|markdown|json|xml|csv|tsv|html|htm|rtf|yaml|yml|log|ini|toml|properties|kt|java|py|js|ts`. PDFs/DOCX/EPUB still get dense-only. `RAGEngine.extractText(bytes, mime, name)` is now exposed in `gguf_lib`, so a future change can route binary docs through it before BM25 — that's a separate workstream.
...
- Don't reintroduce `chatId:sourceId` as the engine's docId in `RagManager`. The engine's docId space is `sourceId` only (with `::ctxN` / `::raptor:lvl…:cluster…` suffixes for sub-docs). The `chatId:sourceId` compound only lives in `chat_documents_meta_v1`'s `id` field, which is the **per-chat-attachment** record. Multiple chats sharing one source = many meta rows, ONE engine entry, ONE BM25 entry, ONE cache blob. Reverting to chatId-prefixed engine ids brings back the duplicate embedding cost and breaks `cache.readBySource` lookups.
- Don't ingest the same `sourceId` into BM25 twice. `indexKeywordsIfTextLike` checks `keywordIndex.docCount(sourceId) > 0` and short-circuits. Likewise the engine path: `if (sourceId in ingestedDocIds) return` before calling `engine.ingestBytes`. Removing those guards re-introduces the duplication the cache is meant to eliminate.
- Don't drop the `RagCacheRepository.invalidateForeign(activeFingerprint)` call in `ensureReady`. When the user switches embedding models (different `model.id`), the on-disk blobs are stale and `engine.importDoc` would return -4. The eager invalidate forces a clean re-embed path via `reembedSource`. Without it, `swapEngineSources` repeatedly hits cache → import → fail → fallback per source on every chat switch.
- Don't write to the cache without the active fingerprint. Every `cache.write(RagCacheEntry(..., fingerprint = activeFingerprint, ...))` must include the fingerprint string the engine produced for the currently loaded model. Storing blank or stale fingerprint defeats the foreign-invalidate sweep and makes the cache poisonous on model swap.
- Don't pass a non-empty `chat_id` to `keywordIndex.ingest(...)` or `keywordIndex.query(...)`. The Kotlin signature dropped the `chatId` parameter precisely so callers can't accidentally re-introduce per-chat duplicates in BM25. The native field is still on disk for binary compatibility; we always write `""` and ignore the C++ filter — Kotlin filters hits by the chat's sourceId set instead.
- Don't restore `rag_vector_snapshot_v1.bin`. That file was plaintext at rest — a real regression vs the rest of the project's encrypted vaults. The encrypted `rag_cache_v1/` HXS vault replaces it. `RagCacheRepository.init` deletes the legacy file on first launch.
- Don't add a `chatId` field to `RagCacheEntry`. The cache is keyed by docId (= `sourceId` or `${sourceId}::ctxN`) and grouped by `sourceId` — chat is irrelevant. Adding chat scope would force per-chat duplication of cache blobs, which is the bug we just removed.
- Don't use `engine.queryFiltered(q, "$chatId:")` in retrieval. The engine docId space no longer carries chatId, so the prefix filter would silently return zero hits. Use `engine.queryFiltered(q, "")` (or `engine.query(q)`) and rely on the per-chat hydrate to keep the engine scoped.
- Don't call `engine.ingestBytes` from inside non-suspend helpers. `ingestBytes` is a `suspend fun` (Dispatchers.IO). The `swapEngineSources` / `ensureSourceInEngine` / `reembedSource` chain is `suspend` for that reason — calls into them must be from inside `opsMutex.withLock { … }` (which is itself suspend) or another suspend context.
- Don't add `nativeRagExportDoc` / `nativeRagImportDoc` to `:app`-side keep rules separately. The existing `-keep class com.dark.gguf_lib.GGUFNativeLib { native <methods>; }` in `consumer-rules.pro` already preserves them, and `-keep class com.dark.gguf_lib.RAGEngine { public *; }` keeps the Kotlin facade. New rules just add noise.
- Don't drop the `synthesizeFollowUp` fallback in `ResearchCoordinator`. After moving to extractive `TextDigest` for compress, smaller chat models often return empty follow-ups at iter 2 (the prompt explicitly tells them "return empty if no useful follow-ups remain"). Without the synthesizer, runs cap at 2 iterations even when `prefs.researchMaxIterations = 5`. The fallback rotates through 6 angle templates (`Latest developments`, `Detailed examples`, `Background and history`, `Different perspectives`, `Specific case studies`, `Common criticism and limitations`) keyed on the original question, skipping ones already in `previousQuestions`. Returns null only when every template has been used — *that* is the legitimate stop condition.
