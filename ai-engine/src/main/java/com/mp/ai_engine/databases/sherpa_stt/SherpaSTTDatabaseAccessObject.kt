...
interface SherpaSTTDatabaseAccessObject {
    ...
    suspend fun getByType(type: ModelType): List<SherpaSTTDatabaseModel>

    @Query("SELECT * FROM openrouter_models WHERE modelType = :type ORDER BY lastUsedAt DESC")
    fun getByTypeFlow(type: ModelType): Flow<List<SherpaSTTDatabaseModel>>

    // ========== QUERY - BY CAPABILITIES ==========

    @Query("SELECT * FROM openrouter_models WHERE supportsTools = 1 ORDER BY lastUsedAt DESC")
    fun getToolsCapableFlow(): Flow<List<SherpaSTTDatabaseModel>>

    @Query("SELECT * FROM openrouter_models WHERE supportsVision = 1 ORDER BY lastUsedAt DESC")
    fun getVisionCapableFlow(): Flow<List<SherpaSTTDatabaseModel>>

    @Query("SELECT * FROM openrouter_models WHERE supportsStreaming = 1 ORDER BY lastUsedAt DESC")
    fun getStreamingCapableFlow(): Flow<List<SherpaSTTDatabaseModel>>

    // ========== QUERY - BY FAVORITES ==========

    @Query("SELECT * FROM openrouter_models WHERE isFavorite = 1 ORDER BY lastUsedAt DESC")
    fun getFavoritesFlow(): Flow<List<SherpaSTTDatabaseModel>>

    @Query("SELECT * FROM openrouter_models WHERE isFavorite = 1 ORDER BY lastUsedAt DESC")
    suspend fun getFavorites(): List<SherpaSTTDatabaseModel>

    // ========== QUERY - BY TAGS ==========

    @Query("SELECT * FROM openrouter_models WHERE tags LIKE '%' || :tag || '%' ORDER BY lastUsedAt DESC")
    fun getByTagFlow(tag: String): Flow<List<SherpaSTTDatabaseModel>>

    @Query("SELECT * FROM openrouter_models WHERE tags LIKE '%' || :tag || '%' ORDER BY lastUsedAt DESC")
    suspend fun getByTag(tag: String): List<SherpaSTTDatabaseModel>
    ...
    """)
    fun getFreeModelsFlow(): Flow<List<SherpaSTTDatabaseModel>>
    ...
    """)
    fun getPaidModelsByPriceFlow(): Flow<List<SherpaSTTDatabaseModel>>
